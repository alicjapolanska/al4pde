...
class PREModel(ProbModel):
    ...
    def train_n_epoch(self, al_iter: int, num_epoch: int, step_offset: int, vis: bool = True, prefix: str = "", is_last=False) -> float:
        total_time = 0
        self.model.init_training(al_iter)
        for i in range(num_epoch):
            t = time.time()
            self.model.train_single_epoch(i, step_offset + i, num_epoch)
            total_time += time.time() - t
            if i % self.model.val_period == 0:
